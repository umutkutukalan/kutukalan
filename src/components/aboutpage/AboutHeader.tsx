...
const AboutHeader = ({ userResponse }: AboutProps) => {
  ...
  return (
    <section className="w-full overflow-hidden relative h-[100vh] bg-[#0e0e10]">
      <div className="relative w-full flex items-end">
        <div className="absolute inset-0 bg-gradient-to-b from-black to-transparent h-150 z-20"></div>
    ...
    [transform:rotateX(-40deg)_rotateY(-20deg)_rotateZ(-32deg)] flex items-end justify-center"
          >
            {[...images].map((img, i) => (
              <li
                key={i}
                className="h-90 w-80 shrink-0 rounded-xl overflow-hidden border border-white/10"
                style={
                  {
